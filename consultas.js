...
export const addPais = (nombre, continente, poblacion, pib_2019, pib_2020) => {
	...
	return new Promise(async (resolve, reject) => {
		try {
			...
			await pool.query(`INSERT INTO paises (nombre, continente, poblacion) VALUES($1, $2, $3)`, [nombre, continente, poblacion]);
			...
			if(paisData.rows.length == 0){
				...
				await pool.query(`INSERT INTO paises_data_web (nombre_pais, accion) VALUES($1, $2)`, [nombre, 1]);
			}else {
				//hacemos update en paises_data_web
				await pool.query(`UPDATE paises_data_web SET accion = 1 WHERE nombre_pais = $1`, [nombre])
			}
...
export const deletePais = (nombre) => {
	return new Promise(async (resolve, reject) => {
		try {
			...
			await pool.query(`DELETE FROM paises WHERE nombre = $1`, [nombre]);
			...
			if(paisData.rows.length == 0){
				//crear registro en paises_data_web
				await pool.query(`INSERT INTO paises_data_web (nombre_pais, accion) VALUES($1, $2)`, [nombre, 0])
			}else{
				//hacemos update en paises_data_web
				await pool.query(`UPDATE paises_data_web SET accion = 0 WHERE nombre_pais = $1`, [nombre])
			}
